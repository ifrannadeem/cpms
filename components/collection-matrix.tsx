...
/**
 * Shared collection matrix (server component) behind both Rent: Collection and
 * Electric: Collection. Cells show money received against each month's invoice;
 * assembly is shared with the Excel export so screen and download always agree.
 */
...
export default async function CollectionMatrix({ reference, assetId, assetName, year, type }: Props) {
  const now = new Date()
  const isElectric = type === 'ELECTRIC'
  const typeLabel = isElectric ? 'Electric' : 'Rent'
  const slug = isElectric ? 'electric-collection' : 'rent-collection'
  const paymentsPath = isElectric ? 'payments-electric' : 'payments'

  const data = await buildRentCollectionData(assetId, reference, assetName, year, type)
  ...
  return (
    <div className="p-6 md:p-10 max-w-none print:p-0">
      <nav className="text-sm text-slate-400 mb-6 flex items-center gap-2 print:hidden">
        ...
        <span>/</span>
        <Link href={`/assets/${reference}`} className="hover:text-slate-600">{assetName}</Link>
        <span>/</span>
        <span className="text-slate-700 font-medium">{typeLabel} Collection</span>
      </nav>
      ...
      <div className="flex items-end justify-between mb-6 flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{assetName} {DASH} {typeLabel} Collection {year}</h1>
          <p className="text-sm text-slate-500 mt-1 print:hidden">
            {isElectric
              ? `Money received against each reading cycle${String.fromCharCode(0x2019)}s invoice (anchored to the cycle end month). Hover a cell for the billed amount.`
        ...
        <div className="flex items-center gap-2 text-sm print:hidden">
          ...
          <a href={`/api/reports/${slug}?assetId=${assetId}&year=${year}`}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6 max-w-3xl print:hidden">
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <p className="text-xs text-slate-400 uppercase tracking-wide mb-1">
            {isElectric ? 'Latest Cycle (billable)' : 'Rent Roll (monthly)'}
          </p>
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white print:overflow-visible print:border-0 print:rounded-none">
        <table className="min-w-full text-sm">
          <thead className="bg-slate-800">
            <tr>
              ...
              <th className="px-3 py-2.5 text-right text-xs font-semibold text-white uppercase tracking-wide">{isElectric ? 'Latest' : 'Rent'}</th>
              {MONTHS.map(mo => (
        ...
        {rows.length === 0 && (
          <div className="p-12 text-center text-slate-400 text-sm">No {typeLabel.toLowerCase()} charges for {year}.</div>
        )}
      </div>
      ...
      <p className="text-xs text-slate-400 mt-3">
        ...
        <span className="print:hidden">
          ...
          <Link href={`/assets/${reference}/${paymentsPath}`} className="text-blue-600 hover:underline">
            {typeLabel}: Payments
          </Link>{' '}page.
        </span>
      </p>
