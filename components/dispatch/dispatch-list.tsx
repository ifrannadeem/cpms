...
export default function DispatchList({ assetId, assetReference, type, month, monthLabel, live, testTo, drafts }: Props) {
  ...
  async function sendOne(d: DraftView): Promise<boolean> {
    if (!canSend(d)) {
  ...
  const sendableCount = drafts.filter(canSend).length
    ...
    <>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-x-6 gap-y-2">
        ...
        >
          ...
          {sendingAll ? 'Sending…' : live ? `Send all (${sendableCount})` : `Send all as test (${sendableCount})`}
        </button>
      </div>
              ...
              <div className="px-5 py-3 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    ...
                    {st.state === 'sending' ? 'Sending…' : st.state === 'sent' ? 'Send again' : live ? 'Send' : 'Send test'}
                  </button>
                </div>
              </div>
