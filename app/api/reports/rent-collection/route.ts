...
/** GET /api/reports/rent-collection?assetId=...&year=YYYY -> xlsx matrix */
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const data = await buildRentCollectionData(assetId, asset.asset_reference, asset.asset_name, year)
    const buf = await buildRentCollectionWorkbook(data)

    const safe = (s: string) => s.replace(/[\\/:*?"<>|]/g, '')
    const filename = `${year} Rent Collection - ${safe(asset.asset_name)}.xlsx`
